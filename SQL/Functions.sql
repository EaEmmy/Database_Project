...
-- SCALAR FUNCTIONS RETURNS SOME VALUES
-- TABLE FUNCTIONS RETURNS A TABLE 


--***************SCALAR FUNCTIONS****************************
GO
CREATE FUNCTION GetInventoryStock(@ProductID varchar(10))
RETURNS int  --returns the stock level for product
AS
BEGIN
DECLARE @ret int;
SELECT @ret = SUM(P_QOH) FROM PRODUCT
WHERE P_CODE = @ProductID
-- want to return a value, even if 0. Don't want to return a null value 
IF (@ret IS NULL)
SET @ret = 0;
...
SELECT * FROM PRODUCT
SELECT P_CODE, DBO.GetInventoryStock('11QER/31')
FROM PRODUCT
WHERE MONTH(P_INDATE) BETWEEN 9 AND 11;

SELECT * FROM PRODUCT
SELECT P_CODE, DBO.GetInventoryStock(P_CODE) AS GETTING_INVETORY_STOCK
FROM PRODUCT
WHERE MONTH(P_INDATE) BETWEEN 9 AND 11;

SELECT * FROM PRODUCT
SELECT P_CODE, DBO.GetInventoryStock('1546-QQ2') AS GETTING_INVETORY_STOCK
FROM PRODUCT
...
SELECT *
FROM DBO.ListWithPrice();

-- ALTER
GO
ALTER FUNCTION ListWithPrice(@IN_PRICE NUMERIC(8,2))
RETURNS TABLE 
AS
RETURN
	SELECT * 
	FROM PRODUCT
	WHERE P_PRICE > @IN_PRICE;

SELECT *
FROM DBO.ListWithPrice(34.5);
